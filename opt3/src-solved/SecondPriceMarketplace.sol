...
contract SecondPriceMarketplace is IERC721Receiver {
    ...
    function createAuction(
        ...
        index++;
        emit NewAuction(index - 1, nftId, msg.sender, initialBid, endAuction);
        return index - 1;
    }
    ...
    function getSecondBid(uint256 auctionIndex) public view returns (uint256) {
        require(auctionIndex < allAuctions.length, "Invalid auction index");
        return allAuctions[auctionIndex].secondBidPrice;
    }
    ...
    function bid(uint256 auctionIndex, uint256 newBid) external returns (bool) {
        ...
        auction.secondBidPrice = auction.currentBidPrice;
    ...
    function claimNFT(uint256 auctionIndex) external {
        ...
        _settle(auction);
        emit NFTClaimed(auctionIndex, auction.nftId, msg.sender, auction.secondBidPrice);
    }
    ...
    function claimToken(uint256 auctionIndex) external {
        ...
        _settle(auction);
        emit TokensClaimed(auctionIndex, auction.nftId, msg.sender, auction.secondBidPrice);
    }
    ...
    function _settle(Auction storage auction) internal {
        ...
        ERC20 paymentToken = ERC20(auction.addressPaymentToken);
        require(paymentToken.transfer(auction.creator, auction.secondBidPrice));
        require(paymentToken.transfer(auction.currentBidOwner, auction.currentBidPrice - auction.secondBidPrice));
    }
